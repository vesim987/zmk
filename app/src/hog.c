...
static struct hids_report layer_input = {
    .id = ZMK_HID_REPORT_ID_LAYER,
    .type = HIDS_INPUT,
};
...
static ssize_t read_hids_layer_input_report(struct bt_conn *conn, const struct bt_gatt_attr *attr,
                                            void *buf, uint16_t len, uint16_t offset) {
    struct zmk_hid_layer_report_body* report_body = &zmk_hid_get_layer_report()->body;
    return bt_gatt_attr_read(conn, attr, buf, len, offset, report_body,
                             sizeof(struct zmk_hid_layer_report_body));
}
...
BT_GATT_SERVICE_DEFINE(
    ...
    BT_GATT_CHARACTERISTIC(BT_UUID_HIDS_REPORT, BT_GATT_CHRC_READ | BT_GATT_CHRC_NOTIFY,
                           BT_GATT_PERM_READ_ENCRYPT, read_hids_layer_input_report, NULL, NULL),
    BT_GATT_CCC(input_ccc_changed, BT_GATT_PERM_READ_ENCRYPT | BT_GATT_PERM_WRITE_ENCRYPT),
    BT_GATT_DESCRIPTOR(BT_UUID_HIDS_REPORT_REF, BT_GATT_PERM_READ_ENCRYPT, read_hids_report_ref,
                       NULL, &layer_input),
...
K_MSGQ_DEFINE(zmk_hog_layer_msgq, sizeof(struct zmk_hid_layer_report_body),
              CONFIG_ZMK_BLE_MOUSE_REPORT_QUEUE_SIZE, 4);

void send_layer_report_callback(struct k_work *work) {
    struct zmk_hid_layer_report_body report;
    while (k_msgq_get(&zmk_hog_mouse_msgq, &report, K_NO_WAIT) == 0) {
        struct bt_conn *conn = destination_connection();
        if (conn == NULL) {
            return;
...
int zmk_hog_send_layer_report(struct zmk_hid_mouse_report_body *report) {
    struct bt_conn *conn = destination_connection();
    if (conn == NULL) {
        return 1;
    }
